...
# Ex08 Разработка класса MyString
...
- **main.cpp** - демонстрационная программа для работы с классом **MyString**.

Файл `MyString.h` помещен в каталог `include`
Файлы `MyString.cpp, main.cpp` помещены в каталог `src` 